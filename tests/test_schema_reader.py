...
@pytest.fixture
def glue_schema_reader(mocked_aws, glue_client):
...
def test_get_polars_schema_correct_for_simple_types(
    mocked_aws, glue_client, glue_db, glue_table_simple, glue_schema_reader
):
...
def test_unsupported_data_type(
    mocked_aws, glue_client, glue_schema_reader, glue_db, glue_table_weird
):
...
def test_table_not_found(mocked_aws, glue_client, glue_schema_reader):
...
def test_split_by_top_level_comma_splits_simple_and_complex(
    mocked_aws, glue_schema_reader
):
    ...
    assert glue_schema_reader.split_by_top_level_comma(type1) == [
        "a:int",
        "b:string",
        "c:float",
    ]
    assert glue_schema_reader.split_by_top_level_comma(type2) == [
        "a:int",
        "b:array<struct<x:int,y:string>>",
        "c:float",
    ]


def test_parse_type_string_handles_simple_types(mocked_aws, glue_schema_reader):
    """Tests that the function handles simple types correctly."""
    type_str = "string"
    assert glue_schema_reader.parse_type_string(type_str) == (type_str, "")


def test_parse_type_string_handles_complex_types(mocked_aws, glue_schema_reader):
    """Tests that the function handles simple types correctly."""
    type_str = "array<string>"
    assert glue_schema_reader.parse_type_string(type_str) == ("array", "string")


def test_parse_type_string_handles_very_complex_types(mocked_aws, glue_schema_reader):
    ...
    assert glue_schema_reader.parse_type_string(type_str) == (
        expected_base,
        expected_content,
    )


def test_get_polars_schema_handles_complex_types(
    mocked_aws, glue_client, glue_schema_reader, glue_db, glue_table_complex
):
    ...
    expected_schema = {
        ...
        "user_profile": pl.Struct(
            [pl.Field("name", pl.Utf8), pl.Field("age", pl.Int32)]
        ),
        "metadata": pl.List(
            pl.Struct([pl.Field("key", pl.Utf8), pl.Field("value", pl.Int64)])
        ),
    }
...
def test_get_polars_schema_handles_very_complex_types(
    mocked_aws, glue_client, glue_schema_reader, glue_db, glue_table_very_complex
):
    """Tests that complex types are handled correctly (e.g., array<string>)."""
...
def test_get_polars_type_correctly_parses_complex_nested_type(
    mocked_aws, glue_schema_reader
):
...
def test_get_polars_type_correctly_parses_very_complex_nested_type(
    mocked_aws, glue_schema_reader
):
